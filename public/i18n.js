...
const translations = {
  en: {
    ...
    'model.how1': 'Each club carries one ELO rating. After every match, the winner gains points and the loser loses them — more if the result was unexpected, and less if xG shows the win was lucky. The gap between two clubs\' ratings gives one set of win/draw/loss odds; a second set comes from each club\'s attack and defence ratings, which move on expected goals (xG) where FotMob has them. The odds shown blend 25 % Elo with 75 % attack/defence. Since Eliteserien 2022, the model uses a faster K-factor and higher xG weight (K=35, α=50 %); earlier seasons and OBOS-ligaen use the legacy config (K=20, α=45 %).',
  ...
  no: {
    ...
    'model.how1': 'Hvert lag har én ELO-rating. Etter hver kamp får vinneren poeng og taperen taper — mer hvis resultatet var overraskende, og mindre hvis xG viser at seieren var heldig. Avstanden mellom to lags ratinger gir ett sett odds for seier, uavgjort og tap; et annet sett kommer fra hvert lags angreps- og forsvarsrating, som oppdateres på forventede mål (xG) der FotMob har dem. Oddsene blander 25 % Elo med 75 % angrep/forsvar. Siden Eliteserien 2022 bruker modellen raskere K-faktor og høyere xG-vekt (K=35, α=50 %); tidligere sesonger og OBOS-ligaen bruker den opprinnelige konfigurasjonen (K=20, α=45 %).',
