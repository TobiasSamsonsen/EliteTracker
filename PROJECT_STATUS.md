...
- [ ] Predicted scorelines on upcoming fixtures. For each unplayed match, show the top few
      likely scorelines with their probabilities (e.g. 2-1 28%, 1-1 19%, 2-0 14%). The
      prediction draws from `model/scorelines.py` conditioned on the win/draw/loss outcome
      and the pre-match rating gap — the same model that feeds the Monte Carlo — so it reads
      straight off the existing three-way odds. Surfaced in the "Next up" section beside the
